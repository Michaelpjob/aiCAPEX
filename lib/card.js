...
const AICAPEX_CARD = (function () {
  ...
  //   1. price_history (array of monthly prices) — when present, draw the full series
  //   2. 3-anchor mode: price_36mo_ago_usd → price_12mo_ago_usd → price_usd
  //   3. 2-anchor fallback: price_12mo_ago_usd → price_usd
  //
  // For 3-anchor mode we ALSO interpolate ~24 monthly points across the two
  // segments so the line has a more graph-like density. The interpolation is
  // linear between anchors, with a tiny deterministic wiggle so the shape
  // isn't perfectly straight. The three anchor points get visible dots.
  function buildSparkline(financials, color) {
    if (!financials) return "";
    const history = financials.price_history;
    const p36 = financials.price_36mo_ago_usd;
    const p12 = financials.price_12mo_ago_usd;
    const p0  = financials.price_usd;

    let pts;        // numeric series
    let anchorIdx;  // indices into pts that are the "real" anchors
    let labels;     // labels under the anchor dots

    if (Array.isArray(history) && history.length >= 2) {
      pts = history.slice();
      anchorIdx = [0, pts.length - 1];
      labels = ['', ''];
    } else if (p36 != null && p12 != null && p0 != null) {
      // 3-anchor mode: build 25 monthly points (12 between 36mo and 12mo,
      // 12 between 12mo and now, plus the now anchor itself)
      pts = [];
      anchorIdx = [];
      // Wiggle is a small per-month perturbation derived from the index so
      // it's deterministic — same data renders the same shape on every load.
      const wiggle = (i) => 1 + 0.012 * Math.sin(i * 1.7) * Math.cos(i * 0.6);
      ...
      for (let i = 1; i < 24; i++) {
        ...
        const seg = (t < 0.5)
          ? p36 + (p12 - p36) * (t / 0.5)
          : p12 + (p0 - p12) * ((t - 0.5) / 0.5);
        pts.push(seg * wiggle(i));
      }
      anchorIdx.push(pts.length);
      pts.push(p0);
      // Insert middle anchor marker at index 12 (halfway)
      anchorIdx.splice(1, 0, 12);
      labels = ['3Y', '1Y', ''];
    } else if (p12 != null && p0 != null) {
      pts = [p12, p0];
      anchorIdx = [0, 1];
      labels = ['1Y', ''];
    } else {
