...
const AICAPEX_CALC = (function () {
  ...
  // ---- Forward price trajectory (current year through endYear) ----
  // Methodology:
  //   Year 2026 anchor = current_price (where the stock is now).
  //   Year y > 2026   = fair_price_2026 × (flow_y / flow_2026)
  //
  //   In words: once the 2026 fair-value gap closes, the stock should track
  //   theoretical-flow growth thereafter (same-multiple assumption). The
  //   discontinuity between 2026 and 2027 represents the modeled re-rating.
  //
  // Each point also carries the year's theoretical capex flow so the chart
  // can overlay the flow shape (bars) alongside the price line.
  //
  // Returns array of { year, price, flow_b, label, type } or null.
  function computeForwardPriceTrajectory(ticker, startYear, endYear) {
    startYear = startYear || 2026;
    endYear   = endYear   || 2029;
    const f = ticker.financials;
    if (!f || f.price_usd == null) return null;

    const fair = computeFairPrice(ticker, startYear);
    const fair_start = fair ? fair.fair_price : f.price_usd;
    const flow_start = computeTheoreticalFlow(ticker, startYear);
    if (flow_start <= 0) return null;
    ...
    for (let y = startYear + 1; y <= endYear; y++) {
      const flow_y = computeTheoreticalFlow(ticker, y);
      const ratio  = flow_y / flow_start;
      series.push({
        year:   y,
        price:  fair_start * ratio,
        flow_b: flow_y,
        label:  String(y),
        type:   'projection'
      });
